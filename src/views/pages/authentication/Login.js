...
const Login = (props) => {
	...
	const [email, setEmail] = useState("admin@livecare.com")
			...
			await useJwt
				...
				.then((res) => {
					...
					if (res.data.status) {
						...
						toast.success(<ToastContentValid name={data.names || "John Doe"} role={data.access || "admin"} />, {
							transition: Slide,
		...
		<div className="auth-wrapper auth-v2">
			<Row className="auth-inner m-0">
				<Link className="brand-logo" to="/" onClick={(e) => e.preventDefault()}>
					...
					<h2 className="brand-text text-primary ml-1">Lifecare Admin</h2>
				</Link>
