...
// ** Store & Actions
import { getUserAllUtilitiesTransactions, getFilteredVendorOrders } from '../../store/action'
...
// custom Header
const CustomHeader = ({ handleFilter, searchTerm, handlePerPage, rowsPerPage, downloadCSV, storeData, downloadPDF, vendorDetails }) => {
	return (
		<div className="invoice-list-table-header w-100 py-2">
			<Row>
				<Col lg="6" sm="12" className="d-flex align-items-center px-0 px-lg-1 mb-2">
					...
					<h3 className="d-none d-lg-block">{vendorDetails?.names?.split(' ')[0]}'s Orders</h3>
				</Col>
				<Col
					lg="3"
					sm="6"
...
const TransactionList = () => {
	...
	useEffect(() => {
		...
		dispatch(
			getFilteredVendorOrders(store.vendorDetails.orders, {
				page: currentPage,
				perPage: rowsPerPage,
				q: searchTerm,
			})
	...
	const filteredData = store?.vendorDetails.orders?.filter((item) => item?.trackingNumber?.toLowerCase() || item?.orderStatus?.name.toLowerCase())
	...
	function convertArrayOfObjectsToCSV(array) {
		...
		const lineDelimiter = '\n'
		const keys = Object.keys(store.vendorDetails.orders[0])
	...
	const downloadPDF = () => {
		...
		store.vendorDetails.orders.map((arr) => {
			doc.autoTable({
				styles: { halign: 'left' },
				columnStyles: {
					0: { cellWidth: 40 },
				...
				body: [[arr.trackingNumber, arr.total, arr.createdAt]],
			})
		})
		doc.save('export.pdf')
	}
	...
	const dataToRender = () => {
		...
		if (store.vendorDetails.orders.length > 0) {
			return store.vendorDetails.orders
		} else if (store.vendorDetails.orders.length === 0 && isFiltered) {
			return []
		} else {
			return store.vendorDetails.orders.slice(0, rowsPerPage)
		}
	}
					...
					<DataTable
						...
						subHeaderComponent={
							<CustomHeader
								...
								storeData={store.vendorDetails.orders}
								downloadPDF={downloadPDF}
								searchTerm={searchTerm}
								vendorDetails={store.vendorDetails}
							/>
						}
					/>
				</div>
