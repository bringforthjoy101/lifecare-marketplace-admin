...
// ** Store & Actions
import { getVendorAllProducts, getFilteredVendorProducts } from '../../store/action'
		...
		<div className="invoice-list-table-header w-100 py-2">
			<Row>
				<Col lg="6" sm="12" className="d-flex align-items-center px-0 px-lg-1 mb-2">
					...
					<h3 className="d-none d-lg-block">{vendorDetails?.names?.split(' ')[0]}'s Products</h3>
				</Col>
				<Col
					lg="3"
					sm="6"
...
const TransactionList = () => {
	...
	const handleFilter = (val) => {
		setSearchTerm(val)
		dispatch(
			getFilteredUserProducts(store.vendorDetails.products, {
				page: currentPage,
				perPage: rowsPerPage,
				q: val,
			})
	...
	const filteredData = store?.vendorDetails.products.filter((item) => item?.productId?.toLowerCase() || item?.status?.toLowerCase())
	...
	function convertArrayOfObjectsToCSV(array) {
		...
		const lineDelimiter = '\n'
		const keys = Object.keys(store.vendorDetails.products[0])
	...
	const downloadPDF = () => {
		...
		store.vendorDetails.products.map((arr) => {
			doc.autoTable({
				styles: { halign: 'left' },
				columnStyles: {
					0: { cellWidth: 40 },
				...
				body: [[arr.id, arr.status, arr.name, arr.createdAt]],
			})
		})
		doc.save('export.pdf')
	}
	...
	const dataToRender = () => {
		...
		if (store.vendorDetails.products.length > 0) {
			return store.vendorDetails.products
		} else if (store.vendorDetails.products.length === 0 && isFiltered) {
			return []
		} else {
			return store.vendorDetails.products.slice(0, rowsPerPage)
		}
	}
					...
					<DataTable
						...
						subHeaderComponent={
							<CustomHeader
								...
								storeData={store.vendorDetails.products}
