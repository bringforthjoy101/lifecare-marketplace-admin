...
import { deleteVendor, getAllData } from '../store/action'
...
const UserInfoCard = ({ vendorDetails, userRole }) => {
	const renderVendorImg = () => {
		if (vendorDetails !== null && vendorDetails.avatars) {
			return <img src={vendorDetails.avatar} alt="user-avatar" className="img-fluid rounded" height="104" width="104" />
		} else {
	...
	const handleDelete = async (id) => {
		return MySwal.fire({
			...
			if (result.value) {
				const deleted = await dispatch(deleteVendor(id))
				if (deleted) {
							...
							<div className="d-flex justify-content-start">
								...
								<div className="d-flex flex-column ml-1">
									<div className="user-info mb-1">
										<h4 className="mb-0">{vendorDetails !== null ? vendorDetails.names : 'Vendor Name'}</h4>
										<CardText tag="span">{vendorDetails?.email}</CardText>
									</div>
									<div className="d-flex flex-wrap align-items-center">
										<Button.Ripple tag={Link} to={`/vendor/edit/${vendorDetails.id}`} disabled color="primary">
											Edit
										</Button.Ripple>
										<Button.Ripple className="ml-1" color="danger" outline onClick={() => handleDelete(vendorDetails.id)}>
											Delete
										</Button.Ripple>
										{/* <Button.Ripple className='ml-1' color='danger' outline onClick={() => handleDelete(vendorDetails.id)}>
                      Delete
						...
						<div className="d-flex align-items-center user-total-numbers">
							<div className="d-flex align-items-center mr-2">
								...
								<div className="ml-1">
									<h5 className="mb-0">
										{vendorDetails?.wallet?.toLocaleString('en-US', { style: 'currency', currency: 'NGN' }) ||
					...
					<Col xl="6" lg="12" className="mt-2 mt-xl-0">
						<div className="user-info-wrapper">
							<div className="d-flex flex-wrap align-items-center mt-0">
								...
								<CardText className="mb-0">{vendorDetails?.email}</CardText>
							</div>
							<div className="d-flex flex-wrap align-items-center mt-1">
								...
								<CardText className="mb-0">{vendorDetails?.phone || 'N/A'}</CardText>
							</div>
							<div className="d-flex flex-wrap align-items-center mt-1">
								<div className="user-info-title">
								...
								<CardText className="text-capitalize mb-0">{vendorDetails?.shops[0].name}</CardText>
							</div>
							<div className="d-flex flex-wrap align-items-center mt-1">
								<div className="user-info-title">
								...
								<CardText className="text-capitalize mb-0">{vendorDetails?.status}</CardText>
							</div>
							<div className="d-flex flex-wrap align-items-center mt-1">
								<div className="user-info-title">
								...
								<CardText className="text-capitalize mb-0">{moment(vendorDetails?.createdAt).format('LL')}</CardText>
							</div>
						</div>
					</Col>
				</Row>
