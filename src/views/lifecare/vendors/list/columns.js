...
import { getAllData, deleteVendor } from '../store/action'
...
const handleDelete = async (id) => {
	...
	return MySwal.fire({
		...
		if (result.value) {
			const deleted = await store.dispatch(deleteVendor(id))
			if (deleted.status) {
		...
		cell: (row) => (
			<div className="d-flex justify-content-left align-items-center">
				...
				<div className="d-flex flex-column">
					<Link to={`/vendors/view/${row.id}`} className="user-name text-truncate mb-0">
	...
	},
	{
		...
		cell: (row) => (
			<UncontrolledDropdown>
				...
				<DropdownMenu right>
					<DropdownItem tag={Link} to={`/vendors/view/${row.id}`} className="w-100">
