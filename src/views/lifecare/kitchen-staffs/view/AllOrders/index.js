...
// ** Store & Actions
import { getUserAllUtilitiesTransactions, getFilteredClientOrders } from '../../store/action'
...
// custom Header
const CustomHeader = ({ handleFilter, searchTerm, handlePerPage, rowsPerPage, downloadCSV, storeData, downloadPDF, clientDetails }) => {
  return (
    <div className='invoice-list-table-header w-100 py-2'>
      <Row>
        <Col lg='6'sm="12" className='d-flex align-items-center px-0 px-lg-1 mb-2'>
          ...
          <h3 className="d-none d-lg-block">{clientDetails.firstName}'s Orders</h3>
        </Col>
        <Col
          lg='3' sm="6"
...
const TransactionList = () => {
  ...
  useEffect(() => {
    ...
    dispatch(
      getFilteredClientOrders(store.clientDetails.orders, {
        page: currentPage,
        perPage: rowsPerPage,
        q: searchTerm
      })
  ...
  const filteredData = store?.clientDetails.orders?.filter(
  ...
  function convertArrayOfObjectsToCSV(array) {
    ...
    const lineDelimiter = '\n'
    const keys = Object.keys(store.clientDetails.orders[0])
  ...
  const downloadPDF = () => {
    ...
    store.clientDetails.orders.map(arr => {
      doc.autoTable({
        styles: { halign: 'left' },
        columnStyles: {
          0: {cellWidth: 40},
  ...
  const dataToRender = () => {
    ...
    if (store.clientDetails.orders.length > 0) {
      return store.clientDetails.orders
    } else if (store.clientDetails.orders.length === 0 && isFiltered) {
      return []
    } else {
      return store.clientDetails.orders.slice(0, rowsPerPage)
    }
  }
          ...
          <DataTable
            ...
            subHeaderComponent={
              <CustomHeader
                ...
                storeData={store.clientDetails.orders}
