...
const UsersList = () => {
  ...
  const filteredData = store.allData.filter( 
    client => (client.otherName?.toLowerCase() || client.firstName?.toLowerCase() || client?.lastName?.toString())
  )
  ...
  return (
    <Fragment>
      ...
      >
        {
          userData?.role === 'manager' || userData?.role === 'busary' ? <Button.Ripple color='primary' onClick={toggleSidebar}> Add New Kitchen Staff </Button.Ripple> : ''
        }
      </Col>
      </Row>
        <DataTable
