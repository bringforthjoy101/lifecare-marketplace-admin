...
export const getFilteredData = (orders, params) => {
  return async dispatch => {
    ...
    const filteredData = orders?.filter(
      order => 
        (order?.orderNumber?.toLowerCase()?.includes(queryLowered) || order?.client.firstName?.toLowerCase()?.includes(queryLowered) || moment(order.createdAt).format('lll').includes(q))
      )
