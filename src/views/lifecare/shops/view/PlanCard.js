...
import { updateShopStatus, deactivateUser, blacklistUserAsset, trackUser, UserDetails } from '../store/action'
			...
			<CardBody>
				{shopDetails.status === 'active' ? (
					<Button.Ripple
						...
						block
						onClick={() => {
							store.dispatch(updateShopStatus(shopDetails.id, 'suspended'))
						}}
					>
						{' '}
						Suspend Shop
					...
					<Button.Ripple
						...
						block
						onClick={() => {
							store.dispatch(updateShopStatus(shopDetails.id, 'active'))
						}}
					>
						Activate Shop
					</Button.Ripple>
				...
				{userData?.role === 'manager' || userData?.role === 'busary' ? (
					<div>
						<AddFunds shopDetails={shopDetails} /> <DeductFunds shopDetails={shopDetails} />{' '}
					</div>
				) : (
					''
				)}
