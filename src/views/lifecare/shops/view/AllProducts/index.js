...
// ** Store & Actions
import { getShopAllProducts, getFilteredShopProducts } from '../../store/action'
		...
		<div className="invoice-list-table-header w-100 py-2">
			<Row>
				<Col lg="6" sm="12" className="d-flex align-items-center px-0 px-lg-1 mb-2">
					...
					<h3 className="d-none d-lg-block">{shopDetails?.name}'s Products</h3>
				</Col>
				<Col
					lg="3"
					sm="6"
...
const TransactionList = () => {
	...
	const handleFilter = (val) => {
		setSearchTerm(val)
		dispatch(
			getFilteredUserProducts(store.shopDetails.products, {
				page: currentPage,
				perPage: rowsPerPage,
				q: val,
			})
	...
	const filteredData = store?.shopDetails.products.filter((item) => item?.productId?.toLowerCase() || item?.status?.toLowerCase())
	...
	function convertArrayOfObjectsToCSV(array) {
		...
		const lineDelimiter = '\n'
		const keys = Object.keys(store.shopDetails.products[0])
	...
	const downloadPDF = () => {
		...
		store.shopDetails.products.map((arr) => {
			doc.autoTable({
				styles: { halign: 'left' },
				columnStyles: {
					0: { cellWidth: 40 },
	...
	const dataToRender = () => {
		...
		if (store.shopDetails.products.length > 0) {
			return store.shopDetails.products
		} else if (store.shopDetails.products.length === 0 && isFiltered) {
			return []
		} else {
			return store.shopDetails.products.slice(0, rowsPerPage)
		}
	}
					...
					<DataTable
						...
						subHeaderComponent={
							<CustomHeader
								...
								storeData={store.shopDetails.products}
