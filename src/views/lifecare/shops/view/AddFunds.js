...
import { getAllData, getCustomerDetails } from '../store/action'

const AddFundsSchema = Yup.object().shape({
	amount: Yup.number().lessThan(100000, `You cant add funds more than 100000`).required('Amount is required').positive().integer(),
	narration: Yup.string().min(3, 'Reason too Short!').max(50, 'Reason too Long!').required('Reason is required'),
