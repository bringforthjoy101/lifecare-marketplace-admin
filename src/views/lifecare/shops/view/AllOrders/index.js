...
// ** Store & Actions
import { getUserAllUtilitiesTransactions, getFilteredShopOrders } from '../../store/action'
...
// custom Header
const CustomHeader = ({ handleFilter, searchTerm, handlePerPage, rowsPerPage, downloadCSV, storeData, downloadPDF, shopDetails }) => {
	return (
		<div className="invoice-list-table-header w-100 py-2">
			<Row>
				<Col lg="6" sm="12" className="d-flex align-items-center px-0 px-lg-1 mb-2">
					...
					<h3 className="d-none d-lg-block">{shopDetails?.names?.split(' ')[0]}'s Orders</h3>
				</Col>
				<Col
					lg="3"
					sm="6"
...
const TransactionList = () => {
	...
	useEffect(() => {
		...
		dispatch(
			getFilteredShopOrders(store.shopDetails.orders, {
				page: currentPage,
				perPage: rowsPerPage,
				q: searchTerm,
			})
	...
	const filteredData = store?.shopDetails.orders?.filter((item) => item?.trackingNumber?.toLowerCase() || item?.orderStatus?.name.toLowerCase())
	...
	function convertArrayOfObjectsToCSV(array) {
		...
		const lineDelimiter = '\n'
		const keys = Object.keys(store.shopDetails.orders[0])
	...
	const downloadPDF = () => {
		...
		store.shopDetails.orders.map((arr) => {
			doc.autoTable({
				styles: { halign: 'left' },
				columnStyles: {
					0: { cellWidth: 40 },
	...
	const dataToRender = () => {
		...
		if (store.shopDetails.orders.length > 0) {
			return store.shopDetails.orders
		} else if (store.shopDetails.orders.length === 0 && isFiltered) {
			return []
		} else {
			return store.shopDetails.orders.slice(0, rowsPerPage)
		}
	}
					...
					<DataTable
						...
						subHeaderComponent={
							<CustomHeader
								...
								storeData={store.shopDetails.orders}
