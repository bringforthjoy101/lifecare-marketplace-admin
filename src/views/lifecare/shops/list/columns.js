...
import { getAllData, deleteShop } from '../store/action'
...
const handleDelete = async (id) => {
	...
	return MySwal.fire({
		...
		if (result.value) {
			const deleted = await store.dispatch(deleteShop(id))
			if (deleted.status) {
				await store.dispatch(getAllData())
				MySwal.fire({
					icon: 'success',
					title: 'Deleted!',
					text: 'Shop record has been deleted.',
					customClass: {
						confirmButton: 'btn btn-primary',
					},
				})
...
export const columns = [
		...
		cell: (row) => (
			<div className="d-flex justify-content-left align-items-center">
				...
				<div className="d-flex flex-column">
					<Link to={`/shops/view/${row.id}`} className="user-name text-truncate mb-0">
	...
	},
	{
		...
		selector: 'type',
		sortable: true,
		cell: (row) => <span className="text-capitalize">{row.phone || 'N/A'}</span>,
	},
	{
		name: 'Products',
		minWidth: '150px',
		selector: 'type',
		sortable: true,
		cell: (row) => <span className="text-capitalize">{row?.products?.length}</span>,
	},
	{
		name: 'Address',
		minWidth: '150px',
	...
	},
	{
		...
		selector: 'type',
		sortable: true,
		cell: (row) => <span className="text-capitalize">{row.location || 'N/A'}</span>,
	},
	...
	},
	{
		...
		cell: (row) => (
			<UncontrolledDropdown>
				...
				<DropdownMenu right>
					<DropdownItem tag={Link} to={`/shops/view/${row.id}`} className="w-100">
