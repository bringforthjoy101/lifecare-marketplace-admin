    ...
    <div className='invoice-list-table-header w-100 py-2'>
      <Row>
        <Col lg='6'sm="12" className='d-flex align-items-center px-0 px-lg-1 mb-2'>
          ...
          <h3 className="d-none d-lg-block">{clientDetails.firstName}'s Transactions</h3>
        </Col>
        <Col
          lg='3' sm="6"
...
const TransactionList = () => {
  ...
  const handleFilter = val => {
    setSearchTerm(val)
    dispatch(
      getFilteredUserTransactions(store.clientDetails.transactions, {
        page: currentPage,
        perPage: rowsPerPage,
        q: val
      })
  ...
  const filteredData = store?.clientDetails.transactions.filter(
  ...
  function convertArrayOfObjectsToCSV(array) {
    ...
    const lineDelimiter = '\n'
    const keys = Object.keys(store.clientDetails.transactions[0])
  ...
  const downloadPDF = () => {
    ...
    store.clientDetails.transactions.map(arr => {
      doc.autoTable({
        styles: { halign: 'left' },
        columnStyles: {
          0: {cellWidth: 40},
  ...
  const dataToRender = () => {
    ...
    if (store.clientDetails.transactions.length > 0) {
      return store.clientDetails.transactions
    } else if (store.clientDetails.transactions.length === 0 && isFiltered) {
      return []
    } else {
      return store.clientDetails.transactions.slice(0, rowsPerPage)
    }
  }
          ...
          <DataTable
            ...
            subHeaderComponent={
              <CustomHeader
                ...
                storeData={store.clientDetails.transactions}
