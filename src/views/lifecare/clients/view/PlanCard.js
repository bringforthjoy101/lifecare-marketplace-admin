...
const PlanCard = ({ clientDetails }) => {
  ...
  return (
    <Card className='plan-card border-primary'>
      ...
      <CardBody>
        {clientDetails.status === "active" ? <Button.Ripple className='text-center mb-1' color= 'danger' outline  block onClick={() => { store.dispatch(updateClientStatus(clientDetails.id, 'inactive')) }}> Deactivate Client</Button.Ripple> : <Button.Ripple 
