...
import { getAllData, deleteService } from '../store/action'
...
const renderClient = row => {
  ...
  if (row.image) {
    ...
    return <Avatar color={color || 'primary'} className='mr-1' content={`${row.name}` || 'Sample Service'} initials />
  }
}
...
const handleDelete = async (id) => {
  ...
  return MySwal.fire({
    ...
    if (result.value) {
      const deleted = await store.dispatch(deleteService(id))
      if (deleted.status) {
