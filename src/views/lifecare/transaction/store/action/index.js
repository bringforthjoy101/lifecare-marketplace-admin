...
export const getFilteredData = (transactions, params) => {
  return async dispatch => {
    ...
    const filteredData = transactions?.filter(
      transaction => 
        (transaction?.transactionId?.toLowerCase()?.includes(queryLowered) || moment(transaction?.createdAt).format('lll')?.toLowerCase()?.includes(queryLowered) || transaction?.client?.firstName?.toLowerCase()?.includes(queryLowered))
      )
