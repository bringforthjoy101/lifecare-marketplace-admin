...
const PlanCard = ({ customerDetails }) => {
			...
			<CardBody>
				{customerDetails.status === 'active' ? (
					<Button.Ripple
						...
						block
						onClick={() => {
							store.dispatch(updateCustomerStatus(customerDetails.id, 'suspended'))
						}}
					>
						{' '}
						Suspend Customer
					...
					<Button.Ripple
						...
						block
						onClick={() => {
							store.dispatch(updateCustomerStatus(customerDetails.id, 'active'))
						}}
					>
						Activate Customer
					</Button.Ripple>
				...
				{userData?.role === 'manager' || userData?.role === 'busary' ? (
					<div>
						<AddFunds customerDetails={customerDetails} /> <DeductFunds customerDetails={customerDetails} />{' '}
					</div>
				) : (
					''
				)}
